...
const generateGalaxy = () => {
  ...
  const vertecies = new Float32Array(parameters.count * 3);
  ...
  for (let i = 0; i < parameters.count; i++) {
    ...
    const randomX =
      Math.abs(Math.pow(Math.random(), parameters.randomnessPower)) *
      (Math.random() < 0.5 ? 1 : -1);
    ...
    vertecies[i3 + 0] = Math.cos(branchAngle + spinAngle) * radius + randomX;
    vertecies[i3 + 1] = randomY;
    vertecies[i3 + 2] = Math.sin(branchAngle + spinAngle) * radius + randomZ;
